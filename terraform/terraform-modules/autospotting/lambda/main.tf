resource "aws_lambda_function" "autospotting" {
  ...
  filename         = "${path.module}/autospotting.zip"
  source_code_hash = "${base64sha256(file("${path.module}/autospotting.zip"))}"
...
resource "aws_lambda_function" "autospotting_from_s3" {
  ...
  depends_on = ["aws_cloudwatch_log_group.log_group_autospotting"]
}

resource "aws_cloudwatch_log_group" "log_group_autospotting" {
  name              = "/aws/lambda/autospotting"
  retention_in_days = 7
}
