...
resource "aws_security_group" "this" {
  name        = "${var.name}-ec2-asg-sg"
  description = "Security group for ${var.name} ec2 asg "
  vpc_id      = "${var.vpc_id}"
...
resource "aws_autoscaling_group" "this" {
  ...
  tags = ["${concat(
    list(
      map("key", "Name", "value", "${var.name}-ec2-asg", "propagate_at_launch", true)
    ),
